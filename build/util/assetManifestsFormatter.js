...
module.exports = (key, value) => {
   ...
   *   }
   */
    Object.keys(manifest).forEach(source => {
        const sourcePath = path.basename(path.dirname(source));
        const targetPath = path.basename(path.dirname(manifest[source]));
        if (sourcePath === targetPath) {
            return;
        }
        manifest[`/${targetPath}/${source}`] = manifest[source];
